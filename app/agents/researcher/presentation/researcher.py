...
from app.agents.researcher.application.use_cases.generate_initial_queries import (
    GenerateInitialQueriesUseCase,
)
from app.agents.researcher.application.use_cases.generate_queries import (
    GenerateQueriesUseCase,
)
from app.agents.researcher.application.use_cases.initialize_research import (
    InitializeResearchUseCase,
)
from app.agents.researcher.application.use_cases.manage_research_state import (
    ManageResearchStateUseCase,
)
from app.agents.researcher.application.use_cases.recover_section_state import (
    RecoverSectionStateUseCase,
)
from app.agents.researcher.application.use_cases.research_section import (
    ResearchSectionUseCase,
)
from app.agents.researcher.application.use_cases.search_web_queries import (
    SearchWebQueriesUseCase,
)
from app.agents.researcher.application.use_cases.validate_query import (
    ValidateQueryUseCase,
)
from app.agents.researcher.application.use_cases.web_search import WebSearchUseCase
from app.agents.researcher.application.use_cases.write_section import (
    WriteSectionUseCase,
)
...
from app.agents.researcher.infrastructure.services.prompt_generation_service import (
...
from app.utils.state import Section, SectionState
...
class ResearchManager:
    def __init__(self, settings=None, repository=None, verbose=False, websocket=None):
        ...
        self.web_searcher = WebSearchUseCase(self.settings.jina_api_key)
        ...
        self.research_section_use_case = ResearchSectionUseCase(
            self.initializer, self.state_manager, self.progress_notifier
        )
        self.generate_initial_queries = GenerateInitialQueriesUseCase(
            self.language_model
        )
    ...
    async def generate_queries(self, state: SectionState) -> dict:
        ...
        try:
            ...
            print(
                f"Evaluating complexity for section: {section.name} and {section.description} in generate_queries to researcher"
            )
            ...
            if complexity_score >= self.grounding_threshold["query_complexity"]:
                ...
                queries_result = (
                    await self.generate_queries_use_case.execute_with_grounding(
                        state, self.grounding_config
                    )
                )
            ...
            await self.progress_notifier.send_progress(
                "Error generating queries", {"error": str(e)}
            )
    ...
    async def write_section(self, state: SectionState) -> dict:
        ...
        try:
            ...
            await self.progress_notifier.send_progress(
                f"Writing section: {section.name}"
            )
            ...
            if section_content:
                ...
                await self.progress_notifier.send_progress(
                    "Section completed", {"section_name": section.name}
                )
                return {"completed_sections": [section]}

        except Exception as e:
            await self.progress_notifier.send_progress(
                "Error writing section", {"error": str(e)}
            )
            raise
    ...
    async def write_section_with_grounding(self, state: SectionState) -> dict:
        ...
        try:
            ...
            await self.progress_notifier.send_progress(
                f"Writing section with grounding: {section.name}"
            )

            grounded_result = await self.section_writer.write_with_grounding(
                section, source_str, self.grounding_config
            )
            ...
            if grounded_result["content"]:
                ...
                if grounded_result.get("grounding_metadata"):
                    setattr(
                        section,
                        "grounding_metadata",
                        grounded_result["grounding_metadata"],
                    )
                logger.debug(
                    f"Completed writing section with grounding: {section.name}"
                )
                await self.progress_notifier.send_progress(
                    "Section completed",
                    {"section_name": section.name, "grounded": True},
                )
                return {
            ...
            await self.progress_notifier.send_progress(
                "Error writing section with grounding", {"error": str(e)}
            )
            raise
    ...
    async def validate_query(self, query: str) -> QueryValidation:
        ...
        try:
            ...
            return QueryValidation(
                specificity=specificity, relevance=relevance, clarity=clarity
            )
        except Exception as e:
            logger.error(f"Query validation failed: {str(e)}")
            # Return default validation instead of failing
            return QueryValidation(specificity=0.7, relevance=0.7, clarity=0.7)
    ...
    async def generate_initial_queries(self, state: SectionState) -> List[str]:
        ...
        return await self.generate_initial_queries.execute(
            state, self.settings.number_of_queries
        )
    ...
    async def write_section_smart(self, state: SectionState) -> dict:
        ...
        try:
            ...
            source_str = state["source_str"]
            print(
                f"Evaluating complexity for section: {section.name} and {section.description} in write_section_smart to researcher"
            )
            ...
            use_grounding = (
                complexity_score >= self.grounding_threshold["query_complexity"]
            )
            ...
            await self.progress_notifier.send_progress(
                "Error in smart section writing", {"error": str(e)}
            )
            raise
